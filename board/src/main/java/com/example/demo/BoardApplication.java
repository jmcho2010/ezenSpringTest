...
//  Pageable, Page 객체 및 부트에서 페이징 처리 관련 객체 정리.
	...
	// 세션과 쿠키
	// 쿠키 : 브라우저에 key-val 쌍으로 로컬에 저장되는 데이터 파일.
	//   -> 계정접속정보 등을 저장하고 있으며 유효기간 내에서는
	//        브라우저가 종료되어도 계속 유지.
	
	// 쿠키의 동작방식
	// 1. 서버가 클라이언트로 부터 요청받으면 정보를 바탕으로 set-cookie 속성을
	//     사용하여 쿠키를 만듬.
	// 2. 서버는 클라이언트에게 해당 요청에 대한 응답을 보낼때 응답의 header 부분에
	//     쿠키를 담아 보냄.(response header)
	// 3. 클라이언트가 응답을 받으면 브라우저는 쿠키를 쿠키 디렉터리에 저장후
	//     또다시 서버에 요청할때 사용자가 별다른 행동을 하지 않아도 
	//     request header에 쿠키를 담아 서버에 전송.
	// 4. 서버는 response에 쿠키 정보를 참조하여 응답.
	
	// 쿠키를 통해 클라이언트가 서버에게 자신을 인증하는 개념.
	...
	// 세션
	//  -> 쿠키를 이용하여 구현.
	//  -> 서버가 클라이언트를 구분하기 위해 응답에 쿠키를 포함할때
	//       sessionID를 부여하고 클라이언트는 쿠키에 session ID를 저장해둔다.
	//  -> 클라이언트의 정보를 서버에 저장하는 특징이있음.

	// 결론 : 세션과 쿠키는 사실 비슷(세션도 쿠키를 활용)
	//  -> 둘은 동작 원리도 거의 비슷
	
	// 시큐리티와 타임리프
	// sec:authorize = "isAnonymous()" :  로그인 되지 않은 경우 해당요소가 표시
	// sec:authorize = "isAuthenticated()"  : 로그인된 경우 해당요소 표시
	
	// DAO, DTO, VO
	// DAO(Data Access Object) : Service단과 DB를 연결하는 역할을 하는 객체(클래스)
	//  -> JPA를 활용할 경우 Repository가 그 역할을 대체한다.
	// DTO(Data Transfer Object) : 데이터의 전송을 위한 객체.
	//  -> DB로 부터 데이터를 가져오거나 혹은 DB에 데이터를 저장할때 사용하는 객체
	// VO(Value Object) : 데이터를 읽어오는 객체.
